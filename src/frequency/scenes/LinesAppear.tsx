...
/**
 * Scene 4 — Lines cascade in with spring physics, cursor clicks Run All,
 * status dots ignite green. At end, cursor clicks "ideas" and camera zooms in.
 */
    ...
    <AbsoluteFill style={{ backgroundColor: colors.bg, overflow: "hidden" }}>
      ...
      <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ width: 1760, height: 840, display: "flex", borderRadius: 14, border: `1px solid ${colors.border}`, overflow: "hidden", boxShadow: "0 16px 48px rgba(0,0,0,0.3)" }}>
            ...
            <div style={{ width: 380, backgroundColor: colors.surface, borderRight: `1px solid ${colors.border}`, display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ padding: "0 24px 14px" }}>
                <div style={{
                  ...
                  border: `1px solid ${clicked ? colors.positive : colors.accent}`, borderRadius: 4,
                  fontFamily: fonts.mono, fontSize: 12, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: "0.05em",
                  color: clicked ? colors.positive : colors.accent,
                  backgroundColor: clicked ? "rgba(48, 209, 88, 0.08)" : colors.accentLight,
                  transform: `scale(${1 + runBtnSpring * 0.06 * (clicked ? 0 : 1)})`,
                  boxShadow: clicked ? `0 0 16px rgba(48, 209, 88, 0.2)` : runBtnSpring > 0.5 ? `0 0 12px rgba(255, 68, 0, 0.15)` : "none",
                }}>
                  <svg width="8" height="8" viewBox="0 0 12 12" fill="none"><path d="M2.5 1.5l7 4.5-7 4.5V1.5z" fill="currentColor" /></svg>
            ...
            {/* Main area */}
            <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "24px 40px", gap: 20 }}>
              <div>
                <h2 style={{ fontFamily: fonts.body, fontSize: 30, fontWeight: 800, color: colors.text, letterSpacing: "-0.03em", margin: 0 }}>app-factory</h2>
                <p style={{ fontFamily: fonts.mono, fontSize: 14, color: colors.accent, marginTop: 4 }}>7 factory lines</p>
              </div>

              <div style={{ display: "flex", gap: 3, marginTop: 12, opacity: pipelineReveal }}>
                {LINES.map((line, i) => {
                  ...
                  return (
                    <div key={line.name} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, opacity: segSpring, transform: `translateY(${interpolate(segSpring, [0, 1], [12, 0])}px)` }}>
                      ...
                      <div style={{ width: "100%", height: 6, backgroundColor: colors.borderLight, borderRadius: 2, overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${fillWidth}%`, backgroundColor: colors.accent, borderRadius: 2 }} />
                      </div>
                      <span style={{ fontFamily: fonts.mono, fontSize: 10, color: colors.textTertiary, textTransform: "uppercase" as const, letterSpacing: "0.12em" }}>{line.name}</span>
                    </div>
                  );
                })}
              </div>
              ...
              {clicked && (
                <div style={{ marginTop: 16, padding: "10px 16px", border: `1px solid rgba(48, 209, 88, 0.2)`, borderRadius: 6, fontFamily: fonts.mono, fontSize: 11, color: colors.positive, backgroundColor: "rgba(48, 209, 88, 0.04)", opacity: statusSpring, transform: `translateY(${interpolate(statusSpring, [0, 1], [8, 0])}px)`, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {/* Cursor — clicks Run All, then clicks "ideas" sidebar */}
        <AnimatedCursor
          waypoints={[
            [960, 540, 0],
